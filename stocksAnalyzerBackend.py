...
class stockTicker():
    ...
    def plot_stock(self, start_date=None, end_date=None, stats=['Adj. Close'], plot_type='basic'):
        ...
        for i, stat in enumerate(stats):
            ...
            print('Current {} = {:.2f} on {}.\n'.format(stat, self.stock.iloc[len(self.stock) - 1].loc[stat], self.max_date.date()))
    ...
    def evaluate_prediction(self, start_date=None, end_date=None, nshares = None):
        ...
        test.loc[(test.y < test.yhat_upper) & (test.y > test.yhat_lower), 'in_range'] = True
    ...
    def changepoint_date_analysis(self, search=None):
        ...
        c_data = train.iloc[change_indices, :].copy()
        ...
        if not search:
            ...
            print(c_data.iloc[0:5].loc[:,['Date', 'Adj. Close', 'delta']])
        ...
        if search:
            ...
            if (trends is None)  or (related_queries is None):
                ...
                return
            if (related_queries[search]['rising'] is None and related_queries[search]['top'] is None):
                print("No search trends found for Search: {} and Range: {}".format(search,date_range))
            else:
                print('\n Top Related Queries: \n')
